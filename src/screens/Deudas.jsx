    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div key={c.id} style={cardStyle}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button type="button" onClick={() => openEditModal(c)} style={{ fontSize: 13, cursor: 'pointer', color: 'var(--text-secondary)' }}>
                  Editar
                </button>
                <button type="button" onClick={() => askDelete(c.id)} style={{ fontSize: 13, cursor: 'pointer', color: 'var(--accent)' }}>
                  Eliminar
                </button>
              </div>
            </div>
      ...
      {modalOpen && (
        <BottomSheet onClose={closeModal}>
          ...
          <input type="number" value={form.balance} onChange={setField('balance')} placeholder="Saldo pendiente" style={textInputStyle()} />
          <input type="date" value={form.nextPayment} onChange={setField('nextPayment')} style={textInputStyle()} />
          <input type="number" value={form.minPayment} onChange={setField('minPayment')} placeholder="Pago mínimo (opcional)" style={textInputStyle()} />
